...
app.post("/songs", (req, res) => {
  // Access the JSON object in the request body
  const { emotion, similar_songs } = req.body;
  emotions = emotion;
  //   console.log("Server response: " + similar_songs);
...
app.get("/create-playlist", async (req, res) => {
  ...
  try {
    ...
    const songs = await getSongsBasedOnEmotion();

    // Create a playlist on Spotify
    const playlistResponse = await createPlaylistOnSpotify(emotions);
...
async function getSongsBasedOnEmotion() {
  ...
  const trackNames = recommendedSongs || [];
...
async function createPlaylistOnSpotify() {
  const playlistName = `Emotion Playlist - ${emotions}`;
  ...
  const createPlaylistResponse = await fetch(
    `https://api.spotify.com/v1/me/playlists`,
    {
      ...
      body: JSON.stringify({
        name: playlistName,
        description: `Playlist created based on the emotion: ${emotions}`,
        public: false,
      }),
    }
  );
